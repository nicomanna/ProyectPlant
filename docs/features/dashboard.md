...
3. Pose inicial fija: el modelo se centra por bounding sphere, la cámara mira de frente a la cara de la maceta y la vista permite rotar (OrbitControls) con zoom restringido y sin pan.
...
## UI / Pantallas
`/` (dashboard): escena immersive glassmorphic 3D (ver `docs/features/ui-redesign-glassmorphism.md`). Fondo gris carbón con neblina volumétrica, la planta 3D centrada como foco (interactiva: rotación libre, zoom restringido, sin pan), los 4 orbes de métricas orbitando alrededor y la meta semanal como panel `glass` flotante arriba a la derecha. Pista "Arrastrá para girar la planta" arriba.
